...
if(isset($_POST['edit_cat'])){
  ...
  if(empty($errors)){
     ...
     if($result && $db->affected_rows() === 1) {
       $session->msg("s", "Categoría atualizada com Sucesso.");
       redirect('categorie.php',false);
     } else {
       $session->msg("d", "Desculpe atualização Falhou.");
       redirect('categorie.php',false);
     }
  } else {
     ...
     <div class="panel panel-default">
       ...
       <div class="panel-body">
         <form method="post" action="edit_categorie.php?id=<?php echo (int)$categorie['id'];?>">
           ...
           <button type="submit" name="edit_cat" class="btn btn-primary">Atualizar categoría</button>
       </form>
       </div>
     </div>
   </div>
