...
<?php
  ...
  if(!$e_user){
    $session->msg("d","ID do usuário ausente.");
    redirect('users.php');
  }
?>
  ...
  if(isset($_POST['update'])) {
    ...
    if(empty($errors)){
       ...
       $status   = remove_junk($db->escape($_POST['status']));
            $sql = "UPDATE users SET user_name ='{$name}', username ='{$username}',user_level='{$level}',status='{$status}' WHERE id='{$db->escape($id)}'";
         $result = $db->query($sql);
...
if(isset($_POST['update-pass'])) {
  ...
  if(empty($errors)){
     ...
     $h_pass   = sha1($password);
       ...
       $result = $db->query($sql);
        if($result && $db->affected_rows() === 1){
          $session->msg('s',"A senha do usuário foi atualizada. ");
          redirect('edit_user.php?id='.(int)$e_user['id'], false);
        } else {
          $session->msg('d','Não foi possível atualizar a senha do usuário..');
          redirect('edit_user.php?id='.(int)$e_user['id'], false);
        }
  } else {
    $session->msg("d", $errors);
       ...
       <div class="panel-body">
          <form method="post" action="edit_user.php?id=<?php echo (int)$e_user['id'];?>" class="clearfix">
            <div class="form-group">
                  ...
                  <input type="name" class="form-control" name="name" value="<?php echo remove_junk(ucwords($e_user['user_name'])); ?>">
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
              <label for="level">Função do usuário</label>
