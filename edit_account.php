...
//update user image
  ...
  if($photo->process_user($user_id)){
    $session->msg('s','A foto foi carregada pra Galeria.');
  ...
  if(isset($_POST['update'])){
    ...
    if(empty($errors)){
       ...
       $username = remove_junk($db->escape($_POST['username']));
            $sql = "UPDATE users SET user_name ='{$name}', username ='{$username}' WHERE id='{$id}'";
    $result = $db->query($sql);
            ...
            <div class="col-md-8">
              ...
              <div class="form-group">
                <input type="hidden" name="user_id" value="<?php echo $user['id'];?>">
                 <button type="submit" name="submit" class="btn btn-danger">Mudar</button>
              </div>
             </form>
            </div>
          </div>
      ...
      <div class="panel-body">
          <form method="post" action="edit_account.php?id=<?php echo (int)$user['id'];?>" class="clearfix">
            <div class="form-group">
                  ...
                  <input type="name" class="form-control" name="name" value="<?php echo remove_junk(ucwords($user['user_name'])); ?>">
            </div>
            <div class="form-group">
